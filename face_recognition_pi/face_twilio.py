...
twilio_number = os.getenv('TWILIO_PHONE_NUMBER', '+15677042619')
target_number = os.getenv('TWILIO_TARGET_NUMBER', '+918760326821')

client = Client(account_sid, auth_token)

def send_alert_sms():
    message = client.messages.create(
        body="ALERT! 3 Wrong Keypad Attempts! Unauthorized entry detected.unauthorized image saved in your file.",
        from_=twilio_number,
        to=target_number
    )
    ...
    while True:
        ...
        if key:
            if key == '#':
                ...
                if attempts == 3:
                    print("ALERT! 3 Wrong Attempts. Unauthorized entry detected!")
                    GPIO.output(BUZZER, GPIO.HIGH)
                    send_alert_sms()  # Twilio SMS
                    return False
